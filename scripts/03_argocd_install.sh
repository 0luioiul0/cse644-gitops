...
run "k apply -n ${ARGO_NS} -f https://raw.githubusercontent.com/argoproj/argo-cd/${ARGOCD_VERSION}/manifests/install.yaml | tail -25"
...
note "ARGOCD_OPTS=--core means the CLI uses the kubeconfig rather than logging"
note "in to argocd-server. No session token and no password appear in any of"
note "these transcripts as a result."
