...
note "The two Secrets that were never in Git went with their namespaces. The"
note "check is scoped by namespace, not by name: Assignment 02 runs a Secret"
note "also called api-secret in the cse644 namespace, and a name-only grep"
note "reports that as a survivor of this cleanup, which it is not."
run "k get secret -A --no-headers 2>/dev/null | awk '\$1 ~ /^cse644-(gitops|monitoring)\$/ {print}' | grep . || echo 'no Secrets remain in either namespace of this assignment'"
note "For contrast, every api-secret on the cluster and where it lives:"
run "k get secret -A --no-headers 2>/dev/null | awk '\$2 == \"api-secret\" {print \"  namespace=\" \$1 \"  name=\" \$2 \"  (Assignment 02, deliberately untouched)\"}'"
