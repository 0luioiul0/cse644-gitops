...
step "Every object carries the Argo CD tracking label"
note "This is how prune knows what it owns. An object created by hand in the"
note "same namespace has no such label and is left alone - which is exactly"
note "what happens to the two Secrets above."
run "kn get deploy,svc,ingress -L app.kubernetes.io/instance"
run "kn get secret api-secret -o jsonpath='{.metadata.labels}'; echo '   <- no instance label: not managed, not pruned'"
