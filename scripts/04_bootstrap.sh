...
note "Argo CD will not admit an Application whose project does not exist, so"
note "the project has to exist before anything else. It lives in"
note "gitops/bootstrap/ and NOT in gitops/apps/, which is the directory the"
note "root Application syncs, and that placement is a correctness requirement"
note "rather than tidiness."
note "When the project was in gitops/apps/ the root Application managed it -"
note "and also used it, because the root Application's own spec.project is"
note "cse644. Deleting the root then deadlocked permanently: its finalizer has"
note "to delete everything it manages, which includes the project, and Argo CD"
note "will not delete a project while an Application still references it. The"
note "referencing Application was the one being deleted. scripts/cleanup.sh"
note "sat at 'applications=1' until it was patched out by hand."
note "Bootstrap objects are created by hand and removed by hand; only the two"
note "child Applications are managed."
run "k apply -f ${ROOT}/gitops/bootstrap/00-project.yaml"
