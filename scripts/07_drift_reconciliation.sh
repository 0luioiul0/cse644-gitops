...
note
note "Waiting for the app to report Synced is not enough either, and this is"
note "the subtle part. .status.sync is written by the comparison loop and says"
note "'live matches revision X' as soon as the comparison says so;"
note ".status.operationState is written by the syncing machinery and says 'the"
note "apply for X actually ran'. Two earlier runs of this script waited on the"
note "first, drifted the cluster, and had the still-queued sync for X wipe it"
note "within eleven seconds - which reads exactly like selfHeal working, and"
note "is the opposite of what this step exists to show. The controller log for"
note "that run says it plainly: 'Initiated automated sync' at the same second"
note "the drift was introduced, with an empty resource list, meaning a full"
note "apply rather than the targeted one a self-heal performs."
stamp "waiting for the sync OPERATION for ${HEAD_SHA:0:8} to complete"
wait_revision cse644-platform "$HEAD_SHA" 90
wait_sync_operation cse644-platform "$HEAD_SHA" 90
run "kargo get application cse644-platform -o jsonpath='{.status.operationState.phase}{\" for \"}{.status.operationState.syncResult.revision}'; echo"
stamp "the apply for ${HEAD_SHA:0:8} has run; nothing is queued"
